...
class VGGNet(nn.Module):
    def __init__(self, num_classes, drop_p=0, drop_last_only=False, feat_dim=1024, conv5=True):
        super(VGGNet, self).__init__()
        if conv5:
            self.features = make_layers(cfg['C5'], True)
        else:
            self.features = make_layers(cfg['C4'], True)
        wh = 1 if conv5 else 2

        layers = list()
        layers.append(nn.Linear(512 * wh * wh, feat_dim))
